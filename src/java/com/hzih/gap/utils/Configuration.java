...
import com.hzih.gap.domain.*;
import com.hzih.gap.entity.ChannelIChangeUtils;
...
public class Configuration {
    ...
    public synchronized void updateQuartzJob(String worktime){
        String start = worktime.split("-")[0];
        String newStart = "0 "+start.split(":")[1]+" "+start.split(":")[0]+" * * ?";
        String end = worktime.split("-")[1];
        String newEnd = "0 "+end.split(":")[1]+" "+end.split(":")[0]+" * * ?";
        Element root = document.getRootElement();
        Element schedule = root.element("schedule");
        boolean start_flag = false;
        boolean end_flag = false;
        List<Element> triggers = schedule.elements("trigger");
        for(int i = 0 ; i < triggers.size() ; i++){
            Element cron = triggers.get(i).element("cron");
            Element cron_expression = cron.element("cron-expression");
            Element name = cron.element("name");
            if(name.getText().endsWith("Start") && cron_expression.getText().equals(newStart)){
                start_flag = true;
            }
            if(name.getText().endsWith("Stop") && cron_expression.getText().equals(newEnd)){
                end_flag = true;
            }
        }
        if(start_flag == false){
            addQuartzJob(newStart,"start");
        }
        if(end_flag == false){
            addQuartzJob(newEnd,"stop");
        }

    }

    private String addQuartzJob(String time,String type){
        String result = "";
        if(type.equals("start")){
            Element schedule = document.getRootElement().element("schedule");
            if(schedule != null){
                Element job = schedule.addElement("job");
                Element child = job.addElement("name");
                String job_name = "Process"+new Date().getTime()+"Start";
                child.setText(job_name);
                child = job.addElement("group") ;
                child.setText("gap");
                child = job.addElement("job-class") ;
                child.setText("com.hzih.gap.core.ProcessStart");
                Element trigger = schedule.addElement("trigger");
                Element cron = trigger.addElement("cron");
                Element child2 = cron.addElement("name");
                child2.setText("Trigger_Process"+new Date().getTime()+"Start");
                child2 = cron.addElement("group");
                child2.setText("gap");
                child2 = cron.addElement("job-name");
                child2.setText(job_name);
                child2 = cron.addElement("job-group");
                child2.setText("gap");
                child2 = cron.addElement("cron-expression");
                child2.setText(time);
            }
        }
        if(type.equals("stop")){
            ...
            if(schedule != null){
                Element job = schedule.addElement("job");
                Element child = job.addElement("name");
                String job_name = "Process"+new Date().getTime()+"Stop";
                child.setText(job_name);
                child = job.addElement("group") ;
                child.setText("gap");
                child = job.addElement("job-class") ;
                child.setText("com.hzih.gap.core.ProcessStop");
                Element trigger = schedule.addElement("trigger");
                Element cron = trigger.addElement("cron");
                Element child2 = cron.addElement("name");
                child2.setText("Trigger_Process"+new Date().getTime()+"Stop");
                child2 = cron.addElement("group");
                child2.setText("gap");
                child2 = cron.addElement("job-name");
                child2.setText(job_name);
...
/***********  操作app。xml   ******************************************************************************************/
    ...
    public String getServerTask(String type){
        Element root = document.getRootElement();
        Element tcp = root.element(type);
        List<Element> servers = tcp.elements("server");
        String json = "{success:true,total:"+servers.size()+",rows:[";
        for(Element server : servers){
            String id = server.attribute("id").getValue();
            String task = server.element("task").getText();
            String ip = server.element("ip").getText();
            String port = server.element("port").getText();
            String tunnel = server.element("tunnel").getText();
            json +="{id:'"+id+"',task:'"+task+"',ip:'"+ip+"',port:'"+port+"',tunnel:'"+tunnel+"'},";
        }
        json += "]}";
        return json;
    }
    ...
    public String getPorts(){
        ...
        String json = "{success:true,total:"+port.size()+",rows:[";
        for(Element p : port){
            ...
            String runstate = p.element("runstate").getText();
            json +="{id:'"+id+"',port:'"+_port+"',worktime:'"+worktime+"',run:'"+run+"',tunnel:'"+tunnel+"',type:'"+type+"',tunneltype:'"+tunneltype+"',runstate:'"+runstate+"'},";
        }
        json += "]}";
        return json;
    }
    ...
    public String clear(){
        ...
        for(Element p : port){
            p.element("runstate").setText("0");
        }
        String json  = saveConnectorAllowIp();
        return json;
    }
...
//        Element root = document.getRootElement();
        for(int i = 0 ; i < data.length ; i++){
            Element port = (Element) document.selectSingleNode("/application/ports/port[@id='"+data[i]+"']");
            if(port != null){
                port.element("runstate").setText("1");
            }
        }
        String json  = saveConnectorAllowIp();
        return json;
    ...
    public String updateStateClear(String[] data){
//        Element root = document.getRootElement();
        for(int i = 0 ; i < data.length ; i++){
            Element port = (Element) document.selectSingleNode("/application/ports/port[@id='"+data[i]+"']");
            if(port != null){
                port.element("runstate").setText("0");
            }
        }
        String json  = saveConnectorAllowIp();
        return json;
    ...
    public String updateStateClearPortEntity(PortEntity entity){
//        Element root = document.getRootElement();
//        for(int i = 0 ; i < data.length ; i++){
            Element port = (Element) document.selectSingleNode("/application/ports/port[@id='"+entity.getId()+"']");
            if(port != null){
                port.element("runstate").setText("0");
            }
//        }
    ...
    public String updateStatePortEntity(PortEntity entity){
//        Element root = document.getRootElement();
//        for(int i = 0 ; i < data.length ; i++){
        Element port = (Element) document.selectSingleNode("/application/ports/port[@id='"+entity.getId()+"']");
        if(port != null){
...
//        }
    ...
    public PortEntity getPortEntity(String id){
        String result = "";
        PortEntity portEntity = new PortEntity();
        Element port = (Element) document.selectSingleNode("/application/ports/port[@id='"+id+"']");

        if(port != null){

            portEntity.setId(id);

            portEntity.setPort(port.element("port").getText());

            portEntity.setRun(port.element("run").getText());

            portEntity.setRunstate(port.element("runstate").getText());

            portEntity.setTunnel(port.element("tunnel").getText());

            portEntity.setTunneltype(port.element("tunneltype").getText());

            portEntity.setType(port.element("type").getText());

            portEntity.setWorktime(port.element("worktime").getText());
    ...
    public boolean checkPort(String Port){
        ...
        for(Element p : port){
            ...
            if(_port.contains("-")){
                ...
                int minPort = Integer.parseInt(_port.split("-")[0]);

                int maxPort = Integer.parseInt(_port.split("-")[1]);

                if(Port.contains("-")){

                    int minPort2 = Integer.parseInt(Port.split("-")[0]);

                    int maxPort2 = Integer.parseInt(Port.split("-")[1]);
                    
                    for(int i = minPort ; i <= maxPort ; i++){
                        
                        for(int j = minPort2 ; j <= maxPort2 ; j++){
                            if(i == j){
                                return false;
                            }
                        }
    ...
    public String addTcpServer(Tcp _tcp){
        String result = "";
        String maxId = getServerMaxId("tcp");
        Element server = (Element) document.selectSingleNode("/application/tcp/server[@id='"+maxId+"']");
        Element tcp = (Element) document.selectSingleNode("/application/tcp");
        if(server == null){
            Element _server = tcp.addElement("server");
            _server.addAttribute("id",maxId);
            Element child = _server.addElement("task");
            child.setText(_tcp.getTask());
            child = _server.addElement("ip") ;
            child.setText(_tcp.getIp());
            child = _server.addElement("port") ;
            child.setText(_tcp.getPort());
            child = _server.addElement("tunnel") ;
            child.setText(_tcp.getTunnel());
        }
        result = saveConnectorAllowIp();
        return result;
    }

    public String updateTcpServer(Tcp _tcp){
        String result = "";
        Element server = (Element) document.selectSingleNode("/application/tcp/server[@id='"+_tcp.getId()+"']");
        if(server != null){
            server.selectSingleNode("task").setText(_tcp.getTask());
            server.selectSingleNode("ip").setText(_tcp.getIp());
            server.selectSingleNode("port").setText(_tcp.getPort());
            server.selectSingleNode("tunnel").setText(_tcp.getTunnel());
        }
        result = saveConnectorAllowIp();
        return result;
    }

    public String deleteTcpServer(Tcp _tcp){
        String result = "";
        Element server = (Element) document.selectSingleNode("/application/tcp/server[@id='"+_tcp.getId()+"']");
        if(server != null){
            Element tcp = (Element) document.selectSingleNode("/application/tcp");
    ...
    public String deletePort(PortEntity portEntity){
        String result = "";
        Element server = (Element) document.selectSingleNode("/application/ports/port[@id='"+portEntity.getId()+"']");
        if(server != null){
            Element tcp = (Element) document.selectSingleNode("/application/ports");
            tcp.remove(server);
        }
        result = saveConnectorAllowIp();
        return result;
    ...
    private String getServerMaxId(String type) {
        Element root = document.getRootElement();
        Element element = root.element(type);
        List<Element> servers = element.selectNodes("server");
        int[] ids = new int[servers.size()];
        for(int i = 0 ; i < ids.length ; i++){
            ids[i] = Integer.parseInt(servers.get(i).attribute("id").getValue());
        }
        if(ids.length == 0){
            return "1";
        }
    ...
    public String addUdpServer(Udp _udp){
        String result = "";
        String maxId = getServerMaxId("udp");
        Element server = (Element) document.selectSingleNode("/application/udp/server[@id='"+maxId+"']");
        Element udp = (Element) document.selectSingleNode("/application/udp");
        if(server == null){
            Element _server = udp.addElement("server");
            _server.addAttribute("id",maxId);
            Element child = _server.addElement("task");
            child.setText(_udp.getTask());
            child = _server.addElement("ip") ;
            child.setText(_udp.getIp());
            child = _server.addElement("port") ;
            child.setText(_udp.getPort());
            child = _server.addElement("tunnel") ;
            child.setText(_udp.getTunnel());
        }
        result = saveConnectorAllowIp();
        return result;
    }

    public String updateUdpServer(Udp _udp){
        String result = "";
        Element server = (Element) document.selectSingleNode("/application/udp/server[@id='"+_udp.getId()+"']");
        if(server != null){
            server.selectSingleNode("task").setText(_udp.getTask());
            server.selectSingleNode("ip").setText(_udp.getIp());
            server.selectSingleNode("port").setText(_udp.getPort());
            server.selectSingleNode("tunnel").setText(_udp.getTunnel());
        }
        result = saveConnectorAllowIp();
        return result;
    }

    public String deleteUdpServer(Udp _udp){
        String result = "";
        Element server = (Element) document.selectSingleNode("/application/udp/server[@id='"+_udp.getId()+"']");
        if(server != null){
            Element tcp = (Element) document.selectSingleNode("/application/udp");
            tcp.remove(server);
        }
        result = saveConnectorAllowIp();
        return result;
    ...
    public String getClientTask(String type){
        Element root = document.getRootElement();
        Element tcp = root.element(type);
        List<Element> servers = tcp.elements("client");
        String json = "{success:true,total:"+servers.size()+",rows:[";
        for(Element server : servers){
            String id = server.attribute("id").getValue();
            String task = server.element("task").getText();
            String ip = server.element("ip").getText();
            String port = server.element("port").getText();
            String start = server.element("start").getText();
            String end = server.element("end").getText();
            String run = server.element("run").getText();
            String tunnel = server.element("tunnel").getText();
            String describe = server.element("describe").getText();
            if(run.equals("true")){
                json +="{id:'"+id+"',task:'"+task+"',ip:'"+ip+"',port:'"+port+"',worktime:'"+start+"-"+end+"',run:'是',tunnel:'"+tunnel+"',describe:'"+describe+"'},";
            }
            else{
                json +="{id:'"+id+"',task:'"+task+"',ip:'"+ip+"',port:'"+port+"',worktime:'"+start+"-"+end+"',run:'否',tunnel:'"+tunnel+"',describe:'"+describe+"'},";
            }
        }
        json += "]}";
        return json;
    }
    public String addTcpClient(Tcp _tcp){
        String result = "";
        String maxId = getClientMaxId("tcp");
        Element client = (Element) document.selectSingleNode("/application/tcp/client[@id='"+maxId+"']");
        Element tcp = (Element) document.selectSingleNode("/application/tcp");
        if(client == null){
            Element _client = tcp.addElement("client");
            _client.addAttribute("id",maxId);
            Element child = _client.addElement("task");
            child.setText(_tcp.getTask());
            child = _client.addElement("ip") ;
            child.setText(_tcp.getIp());
            child = _client.addElement("port") ;
            child.setText(_tcp.getPort());
            child = _client.addElement("start") ;
            child.setText(_tcp.getStart());
            child = _client.addElement("end") ;
            child.setText(_tcp.getEnd());
            child = _client.addElement("run") ;
            child.setText(_tcp.getRun());
            child = _client.addElement("tunnel") ;
            child.setText(_tcp.getTunnel());
            child = _client.addElement("describe") ;
            child.setText(_tcp.getDescribe());
        }
        result = saveConnectorAllowIp();
        return result;
    }
    public String updateTcpClient(Tcp _tcp){
        String result = "";
        Element client = (Element) document.selectSingleNode("/application/tcp/client[@id='"+_tcp.getId()+"']");
        if(client != null){
            client.selectSingleNode("task").setText(_tcp.getTask());
            client.selectSingleNode("ip").setText(_tcp.getIp());
            client.selectSingleNode("port").setText(_tcp.getPort());
            client.selectSingleNode("start").setText(_tcp.getStart());
            client.selectSingleNode("end").setText(_tcp.getEnd());
            client.selectSingleNode("run").setText(_tcp.getRun());
            client.selectSingleNode("tunnel").setText(_tcp.getTunnel());
            client.selectSingleNode("describe").setText(_tcp.getDescribe());
        }
        result = saveConnectorAllowIp();
        return result;
    }
    public String deleteTcpClient(Tcp _tcp){
        String result = "";
        Element client = (Element) document.selectSingleNode("/application/tcp/client[@id='"+_tcp.getId()+"']");
        if(client != null){
            Element tcp = (Element) document.selectSingleNode("/application/tcp");
    ...
    private String getClientMaxId(String type) {
        Element root = document.getRootElement();
        Element element = root.element(type);
        List<Element> clients = element.selectNodes("client");
        int[] ids = new int[clients.size()];
        for(int i = 0 ; i < ids.length ; i++){
            ids[i] = Integer.parseInt(clients.get(i).attribute("id").getValue());
        }
        if(ids.length == 0){
            return "1";
        }
    ...
    public String addUdpClient(Udp _udp){
        String result = "";
        String maxId = getClientMaxId("udp");
        Element client = (Element) document.selectSingleNode("/application/udp/client[@id='"+maxId+"']");
        Element udp = (Element) document.selectSingleNode("/application/udp");
        if(client == null){
            Element _client = udp.addElement("client");
            _client.addAttribute("id",maxId);
            Element child = _client.addElement("task");
            child.setText(_udp.getTask());
            child = _client.addElement("ip") ;
            ...
            child.setText(_udp.getPort());
            child = _client.addElement("start") ;
            child.setText(_udp.getStart());
            child = _client.addElement("end") ;
            child.setText(_udp.getEnd());
            child = _client.addElement("run") ;
            child.setText(_udp.getRun());
            child = _client.addElement("tunnel") ;
            child.setText(_udp.getTunnel());
            child = _client.addElement("describe") ;
            child.setText(_udp.getDescribe());
        }
        result = saveConnectorAllowIp();
        return result;
    }

    public String addPort(PortEntity portEntity){
        String result = "";
        Element port = (Element) document.selectSingleNode("/application/ports/port[@id='"+portEntity.getId()+"']");
        Element ports = (Element) document.selectSingleNode("/application/ports");
        if(port == null ){
            if(checkPort(portEntity.getPort())){
                Element _client = ports.addElement("port");
                _client.addAttribute("id",portEntity.getId());
                Element child = _client.addElement("port");
                child.setText(portEntity.getPort());
                child = _client.addElement("worktime") ;
                child.setText(portEntity.getWorktime());
                child = _client.addElement("run") ;
                child.setText(portEntity.getRun());
                child = _client.addElement("type") ;
                child.setText(portEntity.getType());
                child = _client.addElement("tunneltype") ;
                child.setText(portEntity.getTunneltype());
                child = _client.addElement("runstate") ;
                child.setText("0");
                child = _client.addElement("tunnel") ;
                child.setText(portEntity.getTunnel());
            }
            else{
                result = "端口保存失败,端口重复";
                return  result;
            ...
            if(port.selectSingleNode("port").getText().equals(portEntity.getPort())){
                port.selectSingleNode("port").setText(portEntity.getPort());
                port.selectSingleNode("worktime").setText(portEntity.getWorktime());
                port.selectSingleNode("run").setText(portEntity.getRun());
                port.selectSingleNode("type").setText(portEntity.getType());
                port.selectSingleNode("tunneltype").setText(portEntity.getTunneltype());
                port.selectSingleNode("tunnel").setText(portEntity.getTunnel());
            }
            else{
                if(checkPort(portEntity.getPort())){
                    port.selectSingleNode("port").setText(portEntity.getPort());
    ...
    public String updateUdpClient(Udp _udp){
        String result = "";
        Element client = (Element) document.selectSingleNode("/application/udp/client[@id='"+_udp.getId()+"']");
        if(client != null){
            client.selectSingleNode("task").setText(_udp.getTask());
            client.selectSingleNode("ip").setText(_udp.getIp());
            client.selectSingleNode("port").setText(_udp.getPort());
            client.selectSingleNode("start").setText(_udp.getStart());
            client.selectSingleNode("end").setText(_udp.getEnd());
            client.selectSingleNode("run").setText(_udp.getRun());
            client.selectSingleNode("tunnel").setText(_udp.getTunnel());
            client.selectSingleNode("describe").setText(_udp.getDescribe());
        }
        result = saveConnectorAllowIp();
        return result;
    }
    public String deleteUdpClient(Udp _udp){
        String result = "";
        Element client = (Element) document.selectSingleNode("/application/udp/client[@id='"+_udp.getId()+"']");
        if(client != null){
            Element tcp = (Element) document.selectSingleNode("/application/udp");
    ...
    public String getControlTask(){
        Element root = document.getRootElement();
        Element tcp = root.element("access");
        List<Element> controls = tcp.elements("control");
        String json = "{success:true,total:"+controls.size()+",rows:[";
        for(Element control : controls){
            String id = control.attribute("id").getValue();
            String clientip = control.element("clientip").getText();
            String clientport = control.element("clientport").getText();
            String targetip = control.element("targetip").getText();
            String targetport = control.element("targetport").getText();
            String allow = control.element("allow").getText();
            if(allow.equals("false")){
                json +="{id:'"+id+"',clientip:'"+clientip+"',clientport:'"+clientport+"',targetip:'"+targetip+"',targetport:'"+targetport+"',allow:'禁止'},";
            }
            else{
                json +="{id:'"+id+"',clientip:'"+clientip+"',clientport:'"+clientport+"',targetip:'"+targetip+"',targetport:'"+targetport+"',allow:'允许'},";
            }
        }
        if(json.endsWith(",")){
    ...
    public String addControl(Control _control){
        String result = "";
        String maxId = getControlMaxid();
        Element control = (Element) document.selectSingleNode("/application/access/control[@id='"+maxId+"']");
        Element access = (Element) document.selectSingleNode("/application/access");
        if(control == null){
            Element element_control = access.addElement("control");
            element_control.addAttribute("id",maxId);
            Element child = element_control.addElement("clientip");
            child.setText(_control.getClientip());
            child = element_control.addElement("clientport") ;
            child.setText(_control.getClientport());
            child = element_control.addElement("targetip") ;
            child.setText(_control.getTargetip());
            child = element_control.addElement("targetport") ;
            child.setText(_control.getTargetport());
            child = element_control.addElement("allow") ;
            child.setText(_control.getAllow());
        }
        result = saveConnectorAllowIp();
        return result;
    }
    public String getControlMaxid(){
        Element root = document.getRootElement();
        Element element = root.element("access");
        List<Element> controls = element.selectNodes("control");
        int[] ids = new int[controls.size()];
        for(int i = 0 ; i < ids.length ; i++){
            ids[i] = Integer.parseInt(controls.get(i).attribute("id").getValue());
        }
        for(int j = 0 ; j < ids.length ; j++){
            for(int k = 0 ; k <= j ;k++){
                if(ids[j]>ids[k]){
    ...
    public String updateControl(Control _control){
        String result = "";
        Element control = (Element) document.selectSingleNode("/application/access/control[@id='"+_control.getId()+"']");
        if(control != null){
            control.selectSingleNode("clientip").setText(_control.getClientip());
            control.selectSingleNode("clientport").setText(_control.getClientport());
            control.selectSingleNode("targetip").setText(_control.getTargetip());
            control.selectSingleNode("targetport").setText(_control.getTargetport());
            control.selectSingleNode("allow").setText(_control.getAllow());
        }
        result = saveConnectorAllowIp();
        return result;
    }
    public String deleteControl(Control _control){
        String result = "";
        Element control = (Element) document.selectSingleNode("/application/access/control[@id='"+_control.getId()+"']");
        if(control != null){
            Element access = (Element) document.selectSingleNode("/application/access");
            access.remove(control);
        }
        result = saveConnectorAllowIp();
        return result;
    }
    ...
    public String saveType(String type){
        Element root = document.getRootElement();
        root.attribute("type").setValue(type);
        String result = saveConnectorAllowIp();
        return result;
    }


    public String saveTunnel(Tunnel tunnel){
        String result = "";
        Element _tunnel = (Element) document.selectSingleNode("/application/tunnels/tunnel[@id='"+tunnel.getId()+"']");
        Element _tunnels = (Element) document.selectSingleNode("/application/tunnels");
        if(_tunnel == null){
            Element element_control = _tunnels.addElement("tunnel");
            element_control.addAttribute("id",tunnel.getId());
            Element child = element_control.addElement("name");
            child.setText(tunnel.getName());
            child = element_control.addElement("localIp") ;
            if(tunnel.getLocalIp() != null && tunnel.getLocalIp() != "") {
                child.setText(tunnel.getLocalIp());
            }
            else{
                child.setText("");
            }
            child = element_control.addElement("targetIp") ;
            if(tunnel.getTargetIp() != null && tunnel.getTargetIp() != "") {
                child.setText(tunnel.getTargetIp());
            }
            else{
                child.setText("");
            }
            child = element_control.addElement("gapip") ;
            if(tunnel.getGapip() != null && tunnel.getGapip() != "") {
                child.setText(tunnel.getGapip());
            }
            else{
                child.setText("");
            }
            child = element_control.addElement("videoip") ;
            if(tunnel.getVideoip() != null && tunnel.getVideoip() != "") {
                child.setText(tunnel.getVideoip());
            }
            else{
                child.setText("");
            }
            ...
            if(tunnel.getGapip() != null && tunnel.getGapip() != "") {
                _tunnel.element("gapip").setText(tunnel.getGapip());
            }
            if(tunnel.getVideoip() != null && tunnel.getVideoip() != "") {
                _tunnel.element("videoip").setText(tunnel.getVideoip());
            }
            if(tunnel.getTargetIp() != null && tunnel.getTargetIp() != "") {
                _tunnel.element("targetIp").setText(tunnel.getTargetIp());
            }
            if(tunnel.getLocalIp() != null && tunnel.getLocalIp() != "") {
                _tunnel.element("localIp").setText(tunnel.getLocalIp());
            }
        }
        result = saveConnectorAllowIp();
        return result;
    ...
    public String getTunnel(){
        Element root = document.getRootElement();
        Element tcp = root.element("tunnels");
        List<Element> controls = tcp.elements("tunnel");
        String json = "{success:true,total:"+controls.size()+",rows:[";
        for(Element control : controls){
            String id = control.attribute("id").getValue();
            String localIp = control.element("localIp").getText();
            String targetIp = control.element("targetIp").getText();
            json +="{id:'"+id+"',localIp:'"+localIp+"',targetIp:'"+targetIp+"'},";
        }
        if(json.endsWith(",")){
            json=json.substring(0,json.length()-1);
        }
    ...
    public String getService(){
        ...
        for(Element control : controls){
            String id = control.attribute("id").getValue();
            String gapip = control.element("gapip").getText();
            String localIp = control.element("localIp").getText();
            String targetIp = control.element("targetIp").getText();
            String videoip = control.element("videoip").getText();
            json +="{id:'"+id+"',gapip:'"+gapip+"',videoip:'"+videoip+"',type:'"+type+"',localIp:'"+localIp+"',targetIp:'"+targetIp+"',emptyValue:'——'},";
        }
        if(json.endsWith(",")){
            json=json.substring(0,json.length()-1);
        }
...
/***********  安全配置   **********************************************************************************************/
    ...
    public List<String> getDenyIPs(){
        List<String> denyIps = new ArrayList<String>();
        try{
            Element valve = (Element) document.selectSingleNode("/Server/Service/Engine/Valve");
            if(valve!=null){
                String ip = valve.attribute("deny").getText();
                String[] ips = ip.split("\\|");
                if(ips.length>1){
                    for (int i = 0; i < ips.length; i ++){
                        denyIps.add(ips[i]);
    ...
    public void addType(Type type, String appType) throws Ex {
        ...
        } else if(appType.equals(Type.s_app_file) || appType.equals(Type.s_app_ftpproxy) || appType.equals(Type.s_app_proxy ) ) {
...
//        }
    ...
    public void editType(Type type, String appType) throws Ex {
        ...
        if (typeNode != null) {
            ...
            }if(appType.equals(Type.s_app_file)||appType.equals(Type.s_app_ftpproxy)||appType.equals(Type.s_app_proxy)){
...
//            output = new XMLWriter(new FileWriter(file), format);
    ...
    public List<AuditReset> getAuditResets() throws Exception {
        ...
        for(Iterator it = auditresets.iterator();it.hasNext();){
            ...
            auditReset.setFileSize(e.element("filesize").getText());
    ...
    public void addAuditReset(List<AuditReset> list) {
        ...
        for (AuditReset a : list ){
            ...
            child = auditreset.addElement("filesize");
            child.setText(String.valueOf(a.getFileSize()));
        }
    }
