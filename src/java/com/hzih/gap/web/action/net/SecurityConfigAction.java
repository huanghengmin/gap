...
public class SecurityConfigAction extends ActionSupport {
    ...
    public String readAllowIps() throws Exception {
        HttpServletRequest request = ServletActionContext.getRequest();
        HttpServletResponse response = ServletActionContext.getResponse();
    ...
    private String read(String path, Integer start, Integer limit) {
        ...
        try {
            Configuration config = new Configuration(path);
            List<String> allow = config.getAllowIPs();
            List<String> deny = config.getDenyIPs();
            String[] array = (String[]) allow.toArray(new String[allow.size()]);
            Arrays.sort(array);

            json = "{success:true,total:"+(allow.size())+",rows:[";
            int count = 0;
            for (int i= 0;i<array.length;i++){
                if(i == start && count < limit){
                    json +="{ip:'"+array[i]+"',port:'any',allow:'allow'},";
                    count ++;
                    start ++;
                }
            }
