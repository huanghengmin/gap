package com.hzih.gap.web.action.monitor;
...
public class MonitorAction extends ActionSupport {
    ...
    public String selectBusiness() throws IOException{
        ...
        String json = null;
        try{
            json = monitorService.selectBusiness(start, limit);
//            logService.newLog("INFO", SessionUtils.getAccount(request).getUserName(),"业务运行监控","读取业务监控信息成功!");
        } catch (Exception e){
            logger.error("业务运行监控",e);
            logService.newLog("ERROR", SessionUtils.getAccount(request).getUserName(),"业务运行监控","读取业务监控信息失败!");
    ...
    public String selectEquipment() throws IOException{
        HttpServletRequest request = ServletActionContext.getRequest();
        HttpServletResponse response = ServletActionContext.getResponse();
        ActionBase actionBase = new ActionBase();
        String result = actionBase.actionBegin(request);
        String json = null;
        try{
            json = monitorService.selectEquipment(start, limit);
//            logService.newLog("INFO", SessionUtils.getAccount(request).getUserName(),"设备运行监控","读取设备监控信息成功!");
        } catch (Exception e){
            logger.error("设备运行监控",e);
            logService.newLog("ERROR", SessionUtils.getAccount(request).getUserName(),"设备运行监控","读取设备监控信息失败!");
            json = "{success:true,total:0,rows:[]}";
        }
        actionBase.actionEnd(response,json);
        return null;
    ...
    public String equipmentRunInfo() throws IOException{
        HttpServletRequest request = ServletActionContext.getRequest();
        HttpServletResponse response = ServletActionContext.getResponse();
        ActionBase actionBase = new ActionBase();
        String result = actionBase.actionBegin(request);
        String json = null;
        try{
            json = monitorService.getRunEquipmentInfo(id);
            logService.newLog("INFO", SessionUtils.getAccount(request).getUserName(),"设备运行监控","读取设备"+equipmentName+"运行信息成功!");
        } catch (Exception e){
            logger.error("设备运行监控",e);
            logService.newLog("ERROR", SessionUtils.getAccount(request).getUserName(),"设备运行监控","读取设备"+equipmentName+"运行信息失败!");
            json = "{success:true,total:0,rows:[]}";
        }
        actionBase.actionEnd(response,json);
        return null;
