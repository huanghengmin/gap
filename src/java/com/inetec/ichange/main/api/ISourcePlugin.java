 ...
 * 源插入件接口，源插入件接口必须实现它。
 * 传输插入件与插入件接口之间的协议如下：
 * 1. 传输插入件实例化输入与输出插入件接口的类对象。输入与输出插入件之
 * 不分先后顺序。
 * 2. 传输插入件调用插入件的init方法。输入与输出插入件之间
 *
 *
 *
 * 不分先后顺序。
 * 3. 传输插入件调用插入件的config方法。
 * 4. 传输插入件调用源插入件的start方法。
 *
...
public interface ISourcePlugin {
     ...
     * @throws com.inetec.common.exception.Ex 当有错误的时候。
     */
     ...
     *                 DataAttributes对象。
     * @throws com.inetec.common.exception.Ex 当有错误出现的时候。
     */
     ...
     * @throws com.inetec.common.exception.Ex 当有错误发生时。
     */
