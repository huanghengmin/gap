...
public class CommandDisposer {
     ...
     * *************************************************************************************
     */
    public CommandDisposer(ChannelInfo channleInfo, boolean isPlatform) {
    ...
    private DataAttributes sendData(InputStream is, DataAttributes props) throws Ex {
        ...
        try {
            ...
            if (m_isPlatform) {
                ...
                http.addRequestHeader(ServiceUtil.HDR_ChangeType, ServiceUtil.Str_Platform);
            } else {
            ...
            if (result == 200) {
                ...
                try {
                    ...
                    throw new Ex().set(EChange.E_UNKNOWN, new Message("Failed to send file:{0} ", Ex.getMessage()));
                }

            } else {
                m_cat.debug("Response code from HTTP Connection indicates an error.");
                throw new Ex().set(EChange.E_UNKNOWN, new Message("HTTP Response Code indicates error: ", result));
            }
        } finally {
            try {
        ...
        for (int i = 0; i < N_RETRIES && !done; i++) {
            ...
            try {
                try {
                    ...
                    throw new Ex().set(E.E_OperationFailed, Ex, new Message("Create connection error!!"));
                }
                ...
                try {
                    ...
                    //http.Disconnect();
                    throw (Ex) new Ex().set(E.E_IOException, null,
                ...
                try {
                    ...
                    throw (Ex) new Ex().set(E.E_OperationFailed, e, new Message("Failed to send control request."));
                }
                ...
                if (result == 200) {
                    ...
                    try {
                        ...
                        //http.Disconnect();
                        throw new Ex().set(E.E_OperationFailed, Ex, new Message("Failed to send file:{0} ", Ex.getMessage()));
                    }
                    ...
                    m_cat.debug("Response code from HTTP Connection indicates an error.");
                    throw new Ex().set(E.E_OperationFailed, new Message("HTTP Response Code indicates error:{0} ", http.responseCode()));
                }
