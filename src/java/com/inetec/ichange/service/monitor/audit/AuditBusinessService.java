 ...
 */
public class AuditBusinessService extends Thread {
    private static final Logger logger = Logger.getLogger(AuditBusinessService.class);
    private static final String ExternalXml = System.getProperty("ichange.home")+"/repository/external/config.xml";
    private static final String ExternalAudit = System.getProperty("ichange.home")+"/audit/business/audit.log";
    private static final String ExternalAuditForSendPath = System.getProperty("ichange.home")+"/audit/business/send";
    ...
    private void work() {
        ...
        if(files.length>0){
            ...
            AuditSourceService auditSourceService = new AuditSourceBusinessServiceImpl();
    ...
    private void checkTime(String externalAudit) {
        ...
        if(hour==0&&min==0&&second<5){
            ...
            try {
                ...
                logger.error("创建业务审计信息文件错误", e);
            }
        }
    }

    public static void main(String[] args){
        AuditBusinessService auditService = new AuditBusinessService();
        auditService.init();
        auditService.start();
    }
}
