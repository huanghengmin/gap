...
import com.inetec.ichange.service.monitor.audit.util.BusinessLogDataBase;
 ...
 */
public class AuditTargetBusinessService extends Thread {
    private static final Logger logger = Logger.getLogger(AuditTargetBusinessService.class);
    ...
    public void process(long time, byte[] buf, long fileSize, long id, int len){
        ...
        if(this.time==0||this.time < time){   //新包开始
            ...
            if(id - lastId == 1){ //是连续的包
                ...
                logger.warn("源端发送的业务审计文件数据和目标端收到的数据存在不一致,存在丢包");
                return;
            }
        }
    ...
    public void receive() {
        ...
        packSize = 0;
        if(buffs.size()>0){
            logger.info("处理业务审计文件...");
            byte[] bytes = buffs.toByteArray();
            buffs.reset();
            new BusinessLogDataBase().putIntoDatabase(bytes);
        }
    }
