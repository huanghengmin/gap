...
Ext.onReady(function(){
    ...
    var record = new Ext.data.Record.create([
        ...
        {name:'internalRecord',  mapping:'internalRecord'},
        {name:'externalRecord',  mapping:'externalRecord'},
        {name:'internalCount',  mapping:'internalCount'},
        {name:'externalCount',  mapping:'externalCount'}
    ]);
	...
	var colM = new Ext.grid.ColumnModel([
        ...
        {header:"业务类型",        dataIndex:"businessType",   align:'center',sortable:true,menuDisabled:true},
        {header:"业务传输文件",    dataIndex:"fileName",        align:'center',sortable:true,menuDisabled:true},
        {header:"业务传输表",       dataIndex:"tableName",   align:'center',sortable:true,menuDisabled:true},
        {header:"业务源端文件大小", dataIndex:"externalCount",  align:'center',sortable:true,menuDisabled:true},
        {header:"业务目标文件大小", dataIndex:"internalCount",  align:'center',sortable:true,menuDisabled:true},
        {header:"业务源端数据条数", dataIndex:"externalRecord",  align:'center',sortable:true,menuDisabled:true},
        {header:"业务目标数据条数", dataIndex:"internalRecord",  align:'center',sortable:true,menuDisabled:true}
    ]);
    ...
    store.load({
        params:{
            start:start,limit:pageSize,type:'internal'
        }
    });
});
...
function export_file(grid,store){
    ...
    if(count==0){
        ...
        for(var i = 0; i < record.length; i++){
            ...
            fileSizes[i] = record[i].get('externalCount');
            ...
            fn:function(e){
                if(e == 'ok'){
                    ...
                    Ext.Ajax.request({
                        ...
                        params :{fileNames : fileNames,fileSizes:fileSizes,businessNames:businessNames,businessTypes:businessTypes},
...
function export_fileAll(grid,store){
    var record = new Ext.data.Record.create([
    ...
    _store.load({params:{appType:'file'}});
...
function download(){
    ...
    Ext.Ajax.request({
        url: '../../AuditCompareAction_download.action',
