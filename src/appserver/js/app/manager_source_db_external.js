            ...
            listeners:{
                blur:function(){
                    ...
                    if(appDesc.length>0){
                        ...
                        myMask.show();
                        Ext.Ajax.request({
                            url:'../../AppTypeAction_checkAddDesc.action',
                            params:{appDesc:appDesc,typeXml:'external'},
                            method:'POST',
                            success:function(action){
                                var json = Ext.decode(action.responseText);
                                var msg = json.msg;
                                ...
                                if(json.msg != '0000'){
                                    Ext.MessageBox.show({
                                        ...
                                        fn:function(e){
                                            if(e=='ok'){
                                                Ext.getCmp('external.appDesc.info').setValue('');
                                            }
                                        }
                                    });
                                }
                    ...
                    if (form1.form.isValid()&&form2.form.isValid()) {
                        ...
                        form2.getForm().submit({
                            ...
                            params:{
                                ...
                                dataPath:dataPath,isActive:false,deleteFile:deleteFile,recover:recover,dbName:dbName
                            },
                            method :'POST',
                            waitTitle :'系统提示',
                            waitMsg :'正在保存,请稍后...',
...
function external_update_db_source_app_win(){
	...
	var storeSource = new Ext.data.Store({
        ...
        listeners : {
			load : function(){
				var value = Ext.getCmp('comboVendor.external.info').getValue();
				Ext.getCmp('comboVendor.external.info').setValue(value);
			}
		}
	});
	storeSource.load();
        ...
        Ext.each(selections,function(item){
            ...
            formPanel = new Ext.form.FormPanel({
				...
				items : [{
                    ...
                    listeners:{
                        blur:function(){
                            var appDescOld = item.data.appDesc;
                            var appDesc = this.getValue();
                            if(appDesc.length>0 && appDesc!=appDescOld){
                                var myMask = new Ext.LoadMask(Ext.getBody(),{
                                    msg:'正在校验,请稍后...',
                                    removeMask:true
                                });
                                myMask.show();
                                Ext.Ajax.request({
                                    url:'../../AppTypeAction_checkUpdateDesc.action',
                                    params:{appDesc:appDesc,typeXml:'internal'},
                                    method:'POST',
                                    success:function(action){
                                        var json = Ext.decode(action.responseText);
                                        var msg = json.msg;
                                        ...
                                        if(json.msg != '0000'){
                                            Ext.MessageBox.show({
                                                ...
                                                msg:'<font color="red">'+json.msg+'</font>',
                                                prompt:false,
                                                animEl:'external.appDesc.update.info',
                                                width:200,
                                                buttons:{'ok':'确定'},
                                                icon:Ext.MessageBox.ERROR,
                                                closable:false,
                                                fn:function(e){
                                                    if(e=='ok'){
                                                        Ext.getCmp('external.appDesc.update.info').setValue('');
                                                    }
                                                }
                                            });
                                        }
                    ...
                    id:'comboVendor.external.info',
                    fieldLabel:"数据源", hiddenName:'typeDB.dbName',value:item.data.dbName,
                    xtype:'combo',
                    mode:'local',
                    emptyText :'--请选择--',
                    editable : false,
