...
Ext.onReady(function() {
    ...
    var proxy = new Ext.data.HttpProxy({
        url:"../../TcpClientAction_getTask.action"
    });
...
function insertGridFormWin(grid,store){
    ...
    if(count==0){
        ...
        for(var i = 0; i < record.length; i++){
            ...
            var run = record[i].get('run');
            var tunnel = record[i].get("tunnel");
            var describe = record[i].get('describe');
            id = record[i].get('id');
            if(ip.length>0){
                dataArray[i] = task+"&"+ip+"&"+port+"&"+worktime+"&"+run+"&"+describe+"&id="+id+"&"+tunnel;
            }
        }
        Ext.MessageBox.show({
            title:'信息',
            ...
            fn:function(e){
                if(e=='ok'){
                    ...
                    Ext.Ajax.request({
                        url : '../../TcpClientAction_save.action',
        ...
        Ext.MessageBox.show({
            ...
            fn:function(e){
                if(e=='ok'){
                    ...
                    myMask.show();
                    Ext.Ajax.request({
                        url : '../../TcpClientAction_delete.action',
